...
window.Groups = (function(){
  function page(){
    ...
    v.innerHTML = `<div class="list">${DB.groups.map(g=>`
      <div class="item" onclick="Groups.open('${g.id}')">
        <div style="font-weight:700">${g.name}</div>
        <div class="small">${g.last||''}</div>
      </div>`).join('')}</div>`;
  }
  function open(id){
    ...
    UI.sheet(`<div class="vstack">
      ...
      <div id="chat" class="vstack" style="max-height:50vh; overflow:auto">
        ${msgs.map(m=>`<div class="card"><b>${m.from}</b><div class="small">${m.ts}</div><div>${m.text}</div></div>`).join('')}
      </div>
